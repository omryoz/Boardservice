...
public class MODXJdbcDAOImpl implements GenericDAO {
	...
	public PlayerProfile selectPlayerProfile(int id) throws Exception{
		...
		if (useIntegrations.equals("Y")) {
			userModxId = WalletAdapter.getUserExternalId(String.valueOf(id));
			log.debug("Inside selectPlayerProfile for userModxId #" + String.valueOf(userModxId));
		}
		
		String query = "select username, photo from " + dbPrefix + "users a " +
				" join " + dbPrefix + "user_attributes b on a.id=b.id " +
				" and a.id = ?";
		...
		try {
			 ...
			 */
			preparedStatement.setInt(1, userModxId);
			/**
			...
			if(resultSet.next())
			{
				...
				log.debug("id " + String.valueOf(userModxId));	
				String user_avatar_path = "";;
				String user_avatar = "";
				user_avatar = resultSet.getString("photo");
